...
function test_seed_ecb() {
    var plainText = "안녕하세요";
    var plainValue = CryptoJS.enc.Utf8.parse(plainText); // 평문

    //! 평문 HEX 값
    const bufferText = Buffer.from(plainText, 'utf8').toString('hex');
...
function test_seed_cbc() {
    ...
    //! iv(초기값)
    var ivText = 'llnoteTest!@#123';
    var ivHex = string_to_utf8_hex_string(ivText);
    var iv = CryptoJS.enc.Hex.parse(ivHex);

    var encrypted = CryptoJS.SEED.encrypt(plainValue, key, { iv:iv, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.ZeroPadding });
    var decrypted = CryptoJS.SEED.decrypt(encrypted, key, { iv:iv, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.ZeroPadding });
    var words = CryptoJS.enc.Base64.parse(encrypted.toString());
    console.log("--------------암호화-------------");
    console.log("공통키 : " + keyText);
    console.log("평문 : " + plainText);
